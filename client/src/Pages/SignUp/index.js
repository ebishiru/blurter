        ...
        <div className="bg-green-400 h-screen flex justify-center items-center">
            <div className="w-full max-w-sm px-12 py-8 border-2 border-black rounded-lg bg-white">
                <form onSubmit={handleSignUp}>
                    ...
                    <input className="px-1 mb-2 w-64 border-2 border-black rounded-sm block" type="text" id="username" required value={inputUsername} onChange={(ev)=>{setInputUsername(ev.target.value)}}></input>
                    <label className="block" htmlFor="password">Password</label>
                    <input className="px-1 mb-2 w-64 border-2 border-black rounded-sm block" type="password" id="password" required value={inputPassword} onChange={(ev)=>{setInputPassword(ev.target.value)}}></input>
                    <label className="block" htmlFor="confirm-password">Confirm Password</label>
                    <input className="px-1 mb-2 w-64 border-2 border-black rounded-sm block" type="password" id="confirm-password" required value={inputConfirmPassword} onChange={(ev)=>{setInputConfirmPassword(ev.target.value)}}></input>
                    <label className="block" htmlFor="email">Email</label>
                    <input className="px-1 mb-2 w-64 border-2 border-black rounded-sm block" type="email" id="email" required value={inputEmail} onChange={(ev)=>{setInputEmail(ev.target.value)}}></input>
                    <label className="block" htmlFor="confirm-email">Confirm Email</label>
                    <input className="px-1 mb-2 w-64 border-2 border-black rounded-sm block" type="email" id="confirm-email" required value={inputConfirmEmail} onChange={(ev)=>{setInputConfirmEmail(ev.target.value)}}></input>
